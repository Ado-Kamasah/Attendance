<template>
  <div class="space-y-8 p-1 sm:p-2 lg:p-4 animate-in fade-in duration-500">
    ...
    <div class="relative bg-white dark:bg-[#071328] border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-6 sm:p-8 shadow-sm overflow-hidden">
      ...
      <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
        <div>
          ...
          <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Assign and manage class representatives for each course.</p>
        </div>
    ...
    <div class="flex flex-wrap gap-4">
      <div class="flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-white dark:bg-[#071328] border border-slate-200 dark:border-slate-800 shadow-sm">
    ...
    <div v-if="store.isLoading && store.allReps.length === 0" class="flex flex-col items-center justify-center py-20">
      ...
      <span class="text-xs font-mono text-slate-500 dark:text-slate-400">LOADING CLASS REPS...</span>
    </div>

    <!-- Empty -->
    <div v-else-if="store.allReps.length === 0" class="bg-white dark:bg-[#071328] border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl p-14 text-center">
      ...
      <div class="p-4 border-b border-slate-100 dark:border-slate-800">
        <div class="relative max-w-sm">
          ...
          <input v-model="search" type="text" placeholder="Search by name or course…" id="classrep-search"
            class="w-full pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-secondary/50" />
        </div>
      </div>
            ...
            <tr v-for="rep in filteredReps" :key="rep.id" class="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
              <td class="py-3.5 px-4">
                <div class="flex items-center gap-2.5">
                  ...
                  <div>
                    <div class="font-semibold text-slate-900 dark:text-slate-100">{{ rep.studentName }}</div>
                    <div class="text-[10px] font-mono text-slate-400 truncate max-w-[140px]">{{ rep.studentEmail || rep.studentId }}</div>
                  </div>
                </div>
              </td>
              <td class="py-3.5 px-4 text-slate-500 dark:text-slate-400">{{ rep.studentProgram || '—' }}</td>
              <td class="py-3.5 px-4">
                ...
                <span class="text-slate-500 dark:text-slate-400 ml-2">{{ rep.courseName }}</span>
              </td>
              <td class="py-3.5 px-4">
                <span class="font-mono text-[11px] font-bold px-2 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">L{{ rep.courseLevel }}</span>
              </td>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" @click.self="closeModal">
      <div class="relative w-full max-w-xl bg-white dark:bg-[#071328] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        <div class="corner-tl absolute top-2 left-2 w-3 h-3 border-t-2 border-l-2 border-secondary/40 pointer-events-none"></div>
        <div class="corner-tr absolute top-2 right-2 w-3 h-3 border-t-2 border-r-2 border-secondary/40 pointer-events-none"></div>

        <div class="p-6 border-b border-slate-100 dark:border-slate-800 flex items-start justify-between">
          <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Designate an enrolled student to manage attendance for this course.</p>
          </div>
        ...
        <div class="p-6 overflow-y-auto flex-1 space-y-5">
          ...
          <div class="space-y-1.5">
            <label class="text-[11px] font-mono uppercase tracking-wider text-slate-500 dark:text-slate-400">
          ...
          <div class="space-y-2">
            ...
            <div class="relative">
              ...
              <input v-model="studentSearch" type="text" placeholder="Search by name, ID or email…" id="modal-student-search"
                class="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 rounded-xl text-xs text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-secondary/50" />
            </div>

            <!-- Student List -->
            <div class="max-h-52 overflow-y-auto rounded-xl border border-slate-200 dark:border-slate-700 divide-y divide-slate-100 dark:divide-slate-800">
              ...
              >
                <div :class="['w-6 h-6 rounded-lg flex items-center justify-center font-bold text-[10px] uppercase flex-shrink-0',
                  form.studentId === s.id ? 'bg-primary text-white dark:bg-secondary dark:text-primary' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'
                ]">{{ initials(s.name) }}</div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-1.5">
                    <span class="text-xs font-bold text-slate-900 dark:text-slate-100 truncate">{{ s.name }}</span>
                ...
                <span class="text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 flex-shrink-0">{{ s.mode || 'Regular' }}</span>
              </div>
        ...
        <div class="p-6 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
          <button @click="closeModal" type="button" class="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">Cancel</button>
    ...
    <div v-if="removeTarget" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" @click.self="removeTarget = null">
      <div class="w-full max-w-md bg-white dark:bg-[#071328] border border-rose-200 dark:border-rose-900/50 rounded-2xl p-6 shadow-2xl">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400">
          Are you sure you want to remove <strong class="text-slate-800 dark:text-slate-200">{{ removeTarget.studentName }}</strong> as class rep for <strong class="text-slate-800 dark:text-slate-200">{{ removeTarget.courseCode }}</strong>?
        </p>
        <div class="flex items-center justify-end gap-3 mt-5">
          <button @click="removeTarget = null" class="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">Cancel</button>
