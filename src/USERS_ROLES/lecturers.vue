    ...
    >
      ...
      <div class="h-18 px-4 flex items-center justify-between border-b border-[#162746] dark:border-[#0e1a30] shrink-0">
        <div class="flex items-center gap-3 overflow-hidden">
          <!-- Staff School Icon -->
          <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 via-emerald-600 to-teal-700 flex items-center justify-center shrink-0 shadow-md shadow-emerald-500/20 ring-1 ring-emerald-400/30">
          ...
          <div v-if="!isCollapsed" class="min-w-0 transition-opacity duration-200">
            ...
            <h2 class="font-mono text-[10px] font-semibold tracking-wider text-emerald-400 uppercase truncate">
              Staff Portal
            </h2>
          </div>
        </div>
            ...
            <li v-for="item in group.items" :key="item.name">
              <a 
                ...
                :class="[
                  activeRoute === item.path
                    ? 'bg-emerald-500/15 text-emerald-300 font-semibold shadow-xs border border-emerald-400/30'
              ...
              >
                <component 
                  ...
                  :class="activeRoute === item.path ? 'text-emerald-400' : 'text-slate-400 group-hover:text-slate-200'" 
                />
                ...
                <span 
                  ...
                  class="ml-auto w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0"
                ></span>
      ...
      <!-- Sidebar Footer / Lecturer Profile -->
      <div class="p-3 border-t border-[#162746] dark:border-[#0e1a30] shrink-0 bg-[#030b19]">
        ...
        >
          <div class="relative shrink-0">
            <img 
              :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(userName)}&background=10b981&color=fff&bold=true`" 
              alt="Lecturer Avatar"
              class="w-9 h-9 rounded-lg object-cover ring-1 ring-emerald-500/40"
            />
            <span class="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-emerald-500 ring-2 ring-[#030b19]"></span>
          </div>

          <div v-if="!isCollapsed" class="min-w-0 flex-1">
            <p class="text-xs font-semibold text-slate-100 truncate group-hover:text-emerald-400 transition-colors">
              {{ userName }}
            </p>
...
const activeRoute = ref(window.location.pathname || '/lecturer-dashboard');
