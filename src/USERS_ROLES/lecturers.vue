    ...
    >
      ...
      >
        <div class="flex items-center gap-3 overflow-hidden" :class="isCollapsed ? 'justify-center' : ''">
          <div class="w-[34px] h-[34px] min-w-[34px] flex items-center justify-center text-emerald-400 bg-emerald-500/10 rounded-lg p-[6px]">
          ...
          <div v-if="!isCollapsed" class="flex flex-col">
            ...
            <h2 class="m-0 whitespace-nowrap text-[0.65rem] font-medium tracking-[0.08em] text-emerald-400 leading-tight uppercase mt-0.5">STAFF PORTAL</h2>
          </div>
        </div>
        <button
        ...
        <div v-for="(group, index) in navGroups" :key="index" class="flex flex-col gap-2">
          ...
          <ul class="list-none p-0 m-0 flex flex-col gap-1">
            <li v-for="item in group.items" :key="item.name" class="relative">
              <a
                :href="item.path"
                class="flex items-center gap-3.5 rounded-[10px] text-slate-400 no-underline font-medium transition-all duration-200 hover:text-white hover:bg-emerald-500/10 group"
                :class="[activeRoute === item.path ? 'text-white bg-emerald-500 shadow-[0_4px_12px_rgba(16,185,129,0.3)]' : '', isCollapsed ? 'p-3 justify-center' : 'px-3.5 py-2.5']"
      ...
      <div class="border-t border-white/5 transition-all duration-300" :class="isCollapsed ? 'px-3 py-6' : 'px-5 py-6'">
        <div class="flex items-center gap-3 p-2 rounded-xl cursor-pointer hover:bg-white/5 group" :class="isCollapsed ? 'justify-center' : ''" @click="navigate('/profile')">
          <div class="w-[38px] h-[38px] min-w-[38px] rounded-[10px] overflow-hidden border-2 border-slate-700 group-hover:border-emerald-500 transition-colors duration-300">
            <img :src="`https://ui-avatars.com/api/?name=${encodeURIComponent(userName)}&background=10b981&color=fff`" alt="User Avatar" class="w-full h-full object-cover" />
          </div>
          <div v-if="!isCollapsed" class="flex flex-col overflow-hidden">
            ...
            <p class="m-0 text-[0.75rem] text-slate-400 whitespace-nowrap">{{ userRole }}</p>
          </div>
        </div>
      </div>
    </aside>
...
const activeRoute = ref('/lecturer-dashboard');
...
const navGroups = [{
  ...
  items: [
    { name: 'Dashboard', path: '/lecturer-dashboard', icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="7" height="9"></rect><rect x="14" y="3" width="7" height="5"></rect><rect x="14" y="12" width="7" height="9"></rect><rect x="3" y="16" width="7" height="5"></rect></svg>` },
    { name: 'My Courses', path: '/lecturer-courses', icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"></path><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"></path></svg>` },
    { name: 'View Attendance', path: '/attendance-view', icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>` },
    { name: 'Course Reports', path: '/lecturer-reports', icon: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="20" x2="18" y2="10"></line><line x1="12" y1="20" x2="12" y2="4"></line><line x1="6" y1="20" x2="6" y2="14"></line></svg>` },
